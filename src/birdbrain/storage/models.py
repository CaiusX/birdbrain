"""Central's tables, plus everything a unit needs re-exported.

The unit-side tables live in :mod:`birdbrain.storage.models_core`; they are
imported here so that ``from birdbrain.storage.models import X`` keeps working
for every X, exactly as before the split.
"""
...
from birdbrain.storage.models_core import (
    AppSettingRow,
...
class WeatherObservationRow(Base):
    ...
    # in-process cache key in birdbrain.weather and keeps the primary key
